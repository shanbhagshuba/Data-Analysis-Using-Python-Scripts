...
def veh_model(WAR_LEVEL_DB_CD,VEH_BASE_MDL_NO):
    if WAR_LEVEL_DB_CD == '995-006' or WAR_LEVEL_DB_CD == '995-091' or WAR_LEVEL_DB_CD == '995-1AT':
        model = "MD"
    elif WAR_LEVEL_DB_CD == '995-011' or WAR_LEVEL_DB_CD == '995-037':
        model = "FCC"
    elif VEH_BASE_MDL_NO.contains('PT126') or VEH_BASE_MDL_NO.contains('PE116'):
        model = "P4"
    elif VEH_BASE_MDL_NO.contains('CA125') or VEH_BASE_MDL_NO.contains('CA113'):
        model = "P3"
    else:
        model = "HD"
    return model    
...
def get_customer_active_units(ed_date, st_date, active_data):
    ...
    result = result.loc[:,("VIN", "CURR_CUST_NAME", "Is_active")]
...
def get_final_files(st_date, ed_date, data_for_active_units, claims,portfolio):
    ...
    total_wty_cost = get_wty_cost_cj(Claim_paid_date, claims,['last_dates','VEH_MODEL'])
    total_wty_claims = get_wty_claims(Claim_paid_date, claims,['last_dates','VEH_MODEL'])
    total_wty = pd.merge(total_wty_cost, total_wty_claims, on= ['Claim_paid_date','VEH_MODEL'])
...
#    final_df.to_excel("Engine Rolling_13_month " +str(datetime.today().date()) + ".xlsx") 
    if((portfolio == "P4_chassis" )| (portfolio == "All_chassis")):
    ...
    top_dealers = pd.merge(top_dealers, claims.loc[:,["DEALER_TYPE_CODE", "DEALER_NAME"]], how= 'left', left_on = 'DEALER_NAME', right_on = 'DEALER_NAME')
    ...
    if( (portfolio == "P4_chassis") | (portfolio == "All_chassis")):
        ...
        seag_info_prior = get_wty_cost_cj(prior_three_month_dates, claims, ['COMPONENT_DESC'])
        seag_info_prior.rename(columns={'Total_Cost': 'Total_Cost_Prior'}, inplace=True)
        seag_info_latest = get_wty_cost_cj(last_three_month_dates, claims, ['COMPONENT_DESC'])
        seag_info_latest.rename(columns={'Total_Cost': 'Total_Cost_Latest'}, inplace=True)    
        seag_info = pd.merge(seag_info_latest, seag_info_prior, on ='COMPONENT_DESC')
...
def rename_columns(data_info, df):
    if(data_info =='total_wty'):
        ...
        df2 = df.rename(columns={'Total_Cost':'Total_Cost_cust','Total_Claims':'Total_Claims_cust', 'Is_active': 'Customer_Active_Units'}) 
        return df2
    elif(data_info== 'top_dealers'):
        df2= df.rename(columns={'Total_Cost':'Total_Cost_dlr','Total_Claims':'Total_Claims_dlr'})
        return df2
    elif(data_info== 'bld_yr'):
        df2= df.rename(columns={'TOTAL_WTY_COST_SUM':'Wty_Cost_by_Bld_Yr','BLD_YR':'Build_Year'})
        return df2
    elif(data_info== 'fail_miles'):
        df2= df.rename(columns={'VIN_ODOMETER':'Failure_miles', 'CLAIM_CD':'Claim_Count_fail_miles',})
        return df2
    elif(data_info== 'mis'):
        df2= df.rename(columns={'TOTAL_WTY_COST_SUM':'Wty_Cost_by_MIS'})
        return df2
    else:        
        return df
